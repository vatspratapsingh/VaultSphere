#!/usr/bin/env python3
"""
VaultSphere Enhanced Multi-Tenant Dataset Generator
Generates separate datasets with anomaly_injected column for tracking injected anomalies.
...
class EnhancedTenantDatasetGenerator:
    def __init__(self):
        self.days_back = 30
    ...
    def _generate_normal_events(self, user_id, user_info, num_events, tenant_config):
        ...
        for _ in range(num_events):
            ...
            status = 'SUCCESS' if random.random() < 0.90 else 'FAIL'
            ...
            events.append({
                ...
                'anomaly_injected': 0  # Normal events are not anomalies
            })
        ...
        for user_id, user_info in anomaly_users:
            ...
            for i in range(num_attempts):
                ...
                logs.append({
                    ...
                    'status': 'FAIL',
                    'ip_address': ip_address,
                    'anomaly_injected': 1  # This is an injected anomaly
                })
                anomaly_count += 1
    ...
    def _inject_suspicious_ip_access(self, users_data, logs):
        ...
        for user_id, user_info in anomaly_users:
            ...
            for _ in range(num_events):
                ...
                status = 'FAIL' if random.random() < 0.4 else 'SUCCESS'
    ...
    def generate_tenant_dataset(self, tenant_key):
        ...
        failed_login_anomalies = self._inject_failed_login_bursts(users_data, logs)
        off_hours_anomalies = self._inject_off_hours_activity(users_data, logs, config)
        tenant_specific_anomalies = self._inject_tenant_specific_anomalies(users_data, logs, tenant_key)
        suspicious_ip_anomalies = self._inject_suspicious_ip_access(users_data, logs)
        
        total_anomalies = failed_login_anomalies + off_hours_anomalies + tenant_specific_anomalies + suspicious_ip_anomalies
        print(f"Injected {total_anomalies} anomalous events:")
        print(f"  • Failed login bursts: {failed_login_anomalies}")
        print(f"  • Off-hours activity: {off_hours_anomalies}")
        print(f"  • Tenant-specific: {tenant_specific_anomalies}")
        print(f"  • Suspicious IP access: {suspicious_ip_anomalies}")
    ...
    def save_to_csv(self, logs, filename):
        ...
        # Reorder columns as specified in the prompt
        column_order = ['timestamp', 'tenant_id', 'user_id', 'resource_id', 'event_type', 'status', 'ip_address', 'anomaly_injected']
        df = df[column_order]
        ...
        print(f"Failure Rate: {(df['status'] == 'FAIL').mean():.1%}")
        print(f"Normal Events: {(df['anomaly_injected'] == 0).sum():,} ({(df['anomaly_injected'] == 0).mean():.1%})")
        print(f"Injected Anomalies: {(df['anomaly_injected'] == 1).sum():,} ({(df['anomaly_injected'] == 1).mean():.1%})")
    ...
    def generate_all_datasets(self):
        ...
        print("🎯 VaultSphere Enhanced Multi-Tenant Dataset Generator")
        print("=" * 60)
        ...
        total_events = len(food_logs) + len(it_logs)
...
def main():
    ...
    generator = EnhancedTenantDatasetGenerator()
    datasets = generator.generate_all_datasets()
    
    print("\n✨ Ready for ML model training and anomaly detection research!")
    print("Files created:")
    print("  • vaultsphere_food.csv - Food Company dataset with anomaly tracking")
    print("  • vaultsphere_it.csv - IT Solutions dataset with anomaly tracking")
    print("\nNew features:")
    print("  • anomaly_injected column (1=anomaly, 0=normal)")
    print("  • Status uses 'FAIL' instead of 'FAILURE'")
    print("  • Column order matches specification")
